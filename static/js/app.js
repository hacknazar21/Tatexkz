 ...
 */
    ...
    (() => {
        ...
        async function calculatePrice(form) {
            ...
            form.querySelectorAll('input[type="radio"]').forEach((radio => {
                if (radio.checked && "document" == radio.value) {
                    ...
                    data.type = 'package'
                    const from = form.querySelector('[name="from"]'), where = form.querySelector('[name="where"]'), height = form.querySelector('[name="height"]'), length = form.querySelector('[name="length"]'), promo = form.querySelector('[name="promo"]'), width = form.querySelector('[name="width"]'), weight = form.querySelector('[name="weight"]');
                    data.from = from.value;
                    data.where = where.value;
                    data.weight = parseFloat(weight.value);
            ...
            if (!result["error"]) {
                ...
                if (!form.querySelector('.errorMsg'))
                    form.querySelector("[data-btncalc]").insertAdjacentHTML("beforeBegin", errorMsg);
                return false;
            }
        }
        document.addEventListener("click", (event => {
            if (event.target.closest("[data-tabs]")) {
            ...
            } else if (event.target.classList.contains("header-search__btn") && (null != document.querySelector("._dynamic_adapt_") || window.innerWidth < 860)) {
                const formInput = document.getElementsByClassName("header-search__form");
                ...
                let sortedData = null;
                arrowUp != targetEl && null != arrowUp ? (arrowUp.classList.remove("_filter-arrow-up"),
                    sortBy = 0) : null;
                arrowDown != targetEl && null != arrowDown ? (arrowDown.classList.remove("_filter-arrow-down"),
                    sortBy = 0) : null;
                sortBy++;
                switch (sortBy) {
                    case 1:
        ...
        window.onload = () => {
            ...
            inputs.forEach((input => {
                ...
                hiddenInput.name = `full_${input.name}`
                input.insertAdjacentElement('afterend', hiddenInput)
                input.addEventListener('input', () => {
                    input.parentElement.querySelector('input[type="hidden"]').value = input.parentElement.querySelector('.iti__selected-dial-code').innerHTML + ' ' + input.value
                })
            }));
            ...
            if (document.querySelector('.order__form')) {
                document.querySelector('.order__form').addEventListener('invalid', function (evt) {
                    const border = evt.target.parentElement.style.borderColor;
